...
from mylib.my_common import MySendEmail, MyCustomException, MyDjangoFilterBackend
...
class ForgotPasswordView(APIView):
    def post(self,request,format=None):
        ...
        try:
            ...
            MySendEmail('Stream Password Reset Code',"forgot.html",data,[email])
...
class ResetEnumeratorPassword(APIView):
    allowed_roles=["V","E"]
    ...
    def check_my_permission(self, obj):
        if obj.role not in self.allowed_roles:
            raise MyCustomException("Admin assisted password reset available for %s"%" and  ".join(["Enumerators","View Only accounts"]))
